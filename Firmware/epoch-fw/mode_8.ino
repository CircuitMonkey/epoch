...
void mode_8_4_update_motors() {  // Plunge
  ...
  mode_8_4_cycle += adv;
  if (mode_8_4_cycle > VIB_PI*1.2) { // Arc over just a bit.
    mode_8_4_cycle = 0.0;
  }
  // calc new motor values
  // vibeVal = (sin(advanceRads) + 1) * vibMax/2
  float wav0 = sin(mode_8_4_cycle);
  float wav1 = sin(mode_8_4_cycle + VIB_PI * 0.25);
  float wav2 = sin(mode_8_4_cycle + VIB_PI * 0.5);
  float wav3 = sin(mode_8_4_cycle + VIB_PI * 0.75);
  float wav4 = sin(mode_8_4_cycle + VIB_PI);
  // note: wav->mot sequence is 0,1,3,2
...
void mode_8_5_update_motors() {  // Draw
  ...
  mode_8_5_cycle += adv;
  if (mode_8_5_cycle > VIB_PI*1.2) { // Arc over just a bit.
    mode_8_5_cycle = 0.0;
  }
  // calc new motor values
  float wav0 = sin(mode_8_5_cycle);
  float wav1 = sin(mode_8_5_cycle + VIB_PI * 0.25);
  float wav2 = sin(mode_8_5_cycle + VIB_PI * 0.5);
  float wav3 = sin(mode_8_5_cycle + VIB_PI * 0.75);
  float wav4 = sin(mode_8_5_cycle + VIB_PI);
